...
### 1.15 External Financial Processing (Outside DVH-IMS System Boundary)

| Attribute | Detail |
|-----------|--------|
| Excluded From | V1.2 (out of scope), V1.3 (not in scope), V1.4 (hard constraint) |
| Reference | `DVH-IMS-V1.2_Tasks_and_Phases.md` — Section 11 |
| Description | Financial execution (payments, disbursements, banking) is performed by external authorities (e.g., Ministry of Finance) and is outside the DVH-IMS system boundary. |

**System Boundary Clarification:**

DVH-IMS **MAY**:
- Record a decision outcome (approved/rejected)
- Register an external reference number provided by the executing authority
- Track a status indicating that financial execution is handled externally

DVH-IMS **WILL NOT**:
- Initiate payments
- Calculate or execute disbursements
- Integrate with banking systems

**Classification:** Item 1.15 is not a candidate for future DVH-IMS core functionality. No schema, workflow, or UI implications exist for this item within DVH-IMS.
...
- **Legal/Policy:** HIGH — Budget ceilings involve financial governance rules. Financial execution (payments, disbursements) is outside the DVH-IMS system boundary and handled by external authorities.
- **Schema/RLS:** LIMITED — Items 1.3, 1.4, and 1.11 may require schema changes for budget tracking. Item 1.15 has no schema impact within DVH-IMS (limited to optional external reference number storage).
- **Workflow:** LIMITED — Items 1.3 and 1.4 may affect workflow. Item 1.15 does not introduce workflow states, as DVH-IMS does not perform financial execution.
...
| Date | Item | Change | Reason | Classification |
|------|------|--------|--------|----------------|
| 2026-02-07 | 1.15 | Re-labeled from "Payment / Disbursement Processing" to "External Financial Processing (Outside DVH-IMS System Boundary)" | System boundary clarification — DVH-IMS does not perform payments or financial disbursements | Corrective clarification (not scope expansion) |
